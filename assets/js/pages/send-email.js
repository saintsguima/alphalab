    ...
    buscarStatusEnvio(apiEndpoint)
        .then(dadosRetornados => {
            if (dadosRetornados) {
                ...
                $('#spnUltimoDisparo').text(formatarDataParaExibicao(dadosRetornados.LastEmail));
                $('#spnProximoDisparo').text(formatarDataParaExibicao(dadosRetornados.NextEmail));
            } else {
...
document.getElementById('cmdOk').addEventListener('click', function () {
    ...
    let endPoint = "";
    endPoint = API_URL + "send/sendmail";
