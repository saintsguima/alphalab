$(document).ready(function () {
    var table = $('#dtExcecao').DataTable({
        lengthChange: false,
        buttons: ['copy', 'excel', 'pdf', 'print']
    }); // Inicializa o DataTable

    table.buttons().container().appendTo('#dtExcecao_wrapper .col-md-6:eq(0)');

    // Chama sua API
    let endPoint = "";
    //endPoint = API_URL + "user/listar/";
    endPoint = API_URL + "excecao/listar";
    $.ajax({
        url: endPoint,
        method: 'POST',
        dataType: 'json',
        success: function (response) {
            if (response.status === 'ok') {
                ...
                table.clear().rows.add(
                    response.excecao.map(function (excecao) {
                        return [
                            excecao.Nome,
                            `<button class="btn btn-sm btn-danger" onclick="ExcluirExcecao(${excecao.Id})">Excluir</button>`
                        ];
                    })
                ).draw();
            } else {
...
document.getElementById('cmdOk').addEventListener('click', function () {
    ...
    endPoint = API_URL + "excecao/incluir";
            ...
            if (data.status === "ok") {
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        window.location.href = `${HOST}${APP_HOST}crud-excecao.php`;
                        return;
                    }
                });
            } else {
...
function ExcluirExcecao(id) {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            let endPoint = API_URL + "excecao/excluir";
            $.ajax({
                url: endPoint,
                method: 'POST',
                dataType: 'json',
