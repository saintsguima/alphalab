$(document).ready(function () {
    var table = $('#dtCRErro').DataTable({
        lengthChange: false,
        buttons: ['copy', 'excel', 'pdf', 'print']
    }); // Inicializa o DataTable

    table.buttons().container().appendTo('#dtCRErro_wrapper .col-md-6:eq(0)');

    // Chama sua API
    let endPoint = "";
    //endPoint = API_URL + "user/listar/";
    endPoint = API_URL + "contasreceber/listar-erro";
    $.ajax({
        url: endPoint,
        method: 'POST',
        dataType: 'json',
        success: function (response) {
            if (response.status === 'ok') {
                ...
                table.clear().rows.add(
                    response.crerros.map(function (crerro) {
                        return [
                            crerro.NomeArquivo,
                            crerro.Linha,
                            crerro.CPFCNPJ,
                            formatarDecimal(crerro.Valor),
                            crerro.Historico,
                            formatarData(crerro.DtInicio),
                            formatarData(crerro.DtFinal),
                            formatarDataHora(crerro.DtCriacao),
                            `<button class="btn btn-sm btn-primary" onclick="excluirErro(${crerro.Id})">Excluir</button>`
                        ];
                    })
                ).draw();
            } else {
...
function formatarData(dataString) {
    if (!dataString) return ''; // Retorna string vazia se a data for nula
...
function excluirErroEmLote() {
    let e = $('#txtWhatDelete').val();
    Swal.fire({
        title: "Tem certeza?",
        text: 'Os registros marcados com ' + e + ' serão excluídos permanentemente!',
        icon: "warning",
        showCancelButton: true,
        confirmButtonText: "Sim, excluir",
        cancelButtonText: "Cancelar"
        ...
        if (result.isConfirmed) {
            let endPoint = API_URL + "contasreceber/excluir-em-lote";
            $.ajax({
                url: endPoint,
                method: 'POST',
                dataType: 'json',
                data: JSON.stringify({ wd: e }),
                contentType: "application/json; charset=utf-8",
                success: function (response) {
                    if (response.status === 'ok') {
                        window.location.href = `${HOST}${APP_HOST}list-cr-erro.php`;
