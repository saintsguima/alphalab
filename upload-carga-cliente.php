...
function redirect_with($params)
{
    ...
    header("Location: crud-carga-cliente.php?$qs");
    exit;
}
...
try {
    $unique = "carga-" . bin2hex(random_bytes(16)); // nome aleatório seguro
} catch (Exception $e) {
    // fallback simples
    $unique = "carga-" . sha1(uniqid('', true));
}
...
// Importa a classe
require_once __DIR__ . '/Classes/ProcessaCarga.php';

// URL da API
$apiUrl = $GLOBALS['API_URL'];

// Cria objeto da classe
$processor = new ProcessaCarga($pdo, $apiUrl);

// Processa o arquivo
$resultCarga = $processor->processCarga($fileUrl);
