...
// Configurações
$UPLOAD_DIR  = __DIR__ . '/uploads-extrato'; // pasta física
$PUBLIC_PATH = 'uploads-extrato';            // rota pública
...
function redirect_with($params)
{
    ...
    header("Location: crud-extrato.php?$qs");
    exit;
}
...
// Importa a classe
require_once __DIR__ . '/Classes/ProcessaExtrato.php';

$BancoLayout = $_POST['ddlBanco'];

// URL da API
$apiUrl = $GLOBALS['API_URL'];

// Cria objeto da classe
$processor = new ProcessaExtrato($pdo, $apiUrl);

// Processa o arquivo
$result = $processor->processFileName($finalName);
if (! $result['ok']) {
    // Falhou — pode ser erro HTTP, cURL ou status lógico da API
    // Você tem: $result['status'] (HTTP), $result['error'] (mensagem), $result['data'] (payload bruto)
    error_log("Falha ao incluir arquivo: HTTP {$result['status']} - {$result['error']}");
    die;
    // trate conforme a sua UI/fluxo
} else {
    // Sucesso
    $novoId        = (int) $result['id'];
    $resultExtrato = $processor->processExtrato($fileUrl, $BancoLayout, $novoId);
    // siga o fluxo com o $novoId
    // ex.: echo "Incluído com Id: {$novoId}";
}
// // Processa o arquivo
